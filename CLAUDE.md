...
## 화면 켜고끄기 토글(배경/블러 등)을 추가할 때

- **토글 상태를 `localStorage`에만 저장하면 안 된다.** OBS는 지금 작업 중인 브라우저와
  완전히 별개의 크롬 프로필(별도 캐시/스토리지)이라 `localStorage`를 전혀 공유하지 않는다.
  패널에서 토글을 켜고 주소창 URL이나 "OBS" 버튼으로 복사한 링크를 그대로 넘겨줘도, OBS
  쪽에서는 그 토글을 전혀 몰라서 항상 기본값으로만 열린다 — "토글을 껐다 켜도 방송 화면이
  안 바뀐다"는 형태로 보고됨(2026-08-14, `sunset-bubbles.html`의 배경/블러 토글에서 실제로
  겪음).
- 그래서 토글은 **`localStorage`(같은 브라우저 재방문용) + URL 쿼리 파라미터(다른
  브라우저/OBS로 들고 갈 용도) 둘 다** 반영해야 한다:
  1. 토글을 누르면 `history.replaceState`로 그 즉시 주소창 URL도 갱신한다(새로고침 없이).
  2. 갤러리/프리셋의 "OBS"·"공유" 버튼이 **이 페이지 자신**으로 연결되는 링크를 만들 때는,
     지금 화면에 적용된 토글 상태를 쿼리로 반드시 붙여서 복사한다(다른 페이지로 가는
     링크에는 붙이지 않는다 — 그 페이지가 모르는 쿼리라 의미 없음).
  3. 페이지 로드 시 URL 쿼리가 있으면 `localStorage`보다 **URL을 우선** 적용한다.
- 검증할 땐 헤드리스 브라우저 테스트에서도 `localStorage`를 공유하는 같은 프로필로만
  확인하면 이 버그를 못 잡는다 — 반드시 **완전히 새 브라우저 컨텍스트/프로필**로 그 URL을
  열어서 상태가 재현되는지 확인할 것(Playwright면 `browser.newContext()`를 별도로 하나 더
  만들어서 검증).
...
- **`database.rules.json` 동기화 필수(2026-08-14, 2026-09-04 갱신)**: 이 파일은 같은
  RTDB를 공유하는 6개 레포(StreamBet-Market, soop-stock-market, interior-3d-viewer,
  streamer-life-game, rocket-game, streamer-gallery)가 전부 바이트 단위로 동일한
  사본을 갖고 있어야 한다 — 아무 레포에서나 재배포하면 그 레포 로컬 파일 내용으로
  서버 규칙이 통째로 덮어써지기 때문. 이 파일을 수정할 때마다 나머지 5개 레포에도
  동일한 변경을 그대로 복사(`cp`)해서 diff 0줄 확인 후 커밋할 것.
